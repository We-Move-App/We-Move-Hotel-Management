...
const RoomManagement = () => {
    ...
    const modalOpenForTwo = async (room) => {
        ...
        setSelectedRoom(room?._id);
        // console.log(room);
        const { data, statusCode, success, error } = await apiCall(`${ENDPOINTS.HOTEL_BOOKINGS}?bookingId=${room?.bookingReference}`, 'GET');
        ...
        if (success) {
            const { assingnedRooms, bookedBy, hotelId, roomTypeId, noOfAdults, checkInDate, checkOutDate, noOfRooms, noOfKids, _id } = data.data.bookings[0];
            ...
            if (typeof bookedBy === 'string' && bookedBy == hotelIdByToken) {
                ...
                const { data, success, statusCode, error } = await apiCall(`${ENDPOINTS.PROFILE}`, 'GET');
    ...
    const bookRoom = async () => {
        const { data, statusCode, error, success } = await apiCall(`${ENDPOINTS.ALLOT_ROOM}?bookingId=${bookingId}`, 'PUT', { body: { roomId: selectedRoom } });
    ...
    const getAllRooms = async () => {
        const status = tabBarData[0].status ? 'all' : tabBarData[1].status ? 'available' : 'booked';
        const { data, statusCode, error, success } = await apiCall(`${ENDPOINTS.GET_ALL_ROOMS}?hotelId=${hotelID}&status=${status}&date=${date}`, 'GET')
    ...
    const handleCheckout = async () => {
        ...
        const { data, statusCode, success, error } = await apiCall(`${ENDPOINTS.UPDATE_ROOM_STATUS}?roomId=${selectedRoom}`, 'PUT', {});
        if (error) {
            console.log(error);
            return;
        }
                    ...
                    <CustomModal isOpen={isModalOne} onClose={closeModalOne}>
                        <div className={styles.modalContentBox}>
                            <div className={styles.modalOneBox}>
                                <CustomInput label={"Booking Id"} type='text' alphaNumeric={true} required={true} value={bookingId} onChange={(e) => setBookingId(e.target.value)} boxStyle={{ width: '100%' }} />
                                <CustomButton buttonText={"Assign Room"} type={'button'} style={{ height: '60px', width: "256px" }} onClick={bookRoom} />
                            </div>
                            {
                                modalOneError && <div className={styles.error} >{modalOneError}</div>
                            }
                        </div>
                    </CustomModal>
                )
                    ...
                    <CustomModal isOpen={isModalTwo} onClose={closeModalTwo}>
                        <div className={styles.modalTwoBox}>
                            ...
                            <div className={styles.checkoutButton} >
                                <CustomButton buttonSize={'small'} buttonText={'Checkout'} onClick={handleCheckout} />
                            </div>
                        </div>
                    </CustomModal>
                )
            ...
            <div className={styles.roomDetailsBox}>
                ...
                <div className={styles.roomDetailsHeader}>
                    ...
                    <div className={styles.datePickerWrapper}>
                        ...
                        {/* <CustomDateInput/> */}
                        <ResponsiveDatePickers textColor='#000000' backgroundColor={GlobalStyles.colorPrimaryLightType4} setDate={setDate} />
                    </div>
                </div>
                <div className={styles.gridBoxWrapper}>
                    ...
                    <div className={styles.typeIdentifierBox}>
                        <div className={styles.reserved}><span></span>Reserved</div>
                        <div className={styles.available}><span></span>Available</div>
                        <div><span>T-Series:</span> Luxury</div>
                        <div><span>G-Series:</span> Standard</div>
                    </div>
                                    ...
                                    <div className={styles.roomGrid}>
                                        {
                                            rooms?.map((room) => (
                                                <div key={room?._id} className={styles.room}>
                                                    <div
                                                        className={`${room.isAvailable ? styles.statusTrue : styles.statusFalse}`}
                                                        onClick={() => {
                                                            room.isAvailable ? modalOpenFor(room) : modalOpenForTwo(room);
                                                        }}
                                                    ></div>
