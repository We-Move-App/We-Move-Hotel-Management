...
const BankDetailsSchema = Yup.object().shape({
    bankName: Yup.string()
        .matches(/^[a-zA-Z\s]*$/, 'Bank Name should only contain alphabets and spaces') // Only alphabets and spaces
    ...
    accountHolderName: Yup.string()
        .matches(/^[a-zA-Z\s]*$/, 'Account Holder Name should only contain alphabets and spaces') // Only alphabets and spaces
        .test('noSpaceAtStart', 'The first character cannot be a space', (value) => value?.trim().charAt(0) !== ' ') // No space at the start
...
const Profile = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const { bankName, bankAccountNumber, accountHolderName, bankDocs } = values;
            ...
            if (bankId) {
                const { data, statusCode, error, success } = await apiCall(`${ENDPOINTS.HOTEL_BANK_DETAILS}/${bankId}`, 'PUT', {
                    body: payloadForm,
                    // headers: {
            ...
            <div className={styles.contentContainer}>
                <div className={styles.personalDetailsContainer}>
                    <div className={styles.profileImageBox}>
                        <img src={user?.avatar || images.profileImage} className={styles.profileImage} alt="profile" />
                    </div>
                            ...
                            : <div className={styles.formWrapper}>
                                <form className={styles.form} onSubmit={formik.handleSubmit} >
                                    ...
                                    <div className={styles.formSubmitBtn}>
                                        {
                                            !isEdit && <CustomButton buttonText={'Edit'} type={'button'} onClick={() => setIsEdit(!isEdit)} buttonSize={'medium'} />
                                        }
                                        {
                                            isEdit && <CustomButton buttonText={bankId ? 'Update' : "Add Bank"} type={'submit'} buttonSize={'medium'} />
                                        }
                                    </div>
                                </form>
                            </div>
