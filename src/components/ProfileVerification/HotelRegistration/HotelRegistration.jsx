...
import { getAmenities, getDataFromLocalStorage, tokenFromLocalStorage } from '../../../utils/helperFunctions';
...
const HotelRegistration = () => {
  ...
  const [multistepTabs, setMultistepTabs] = useState([
    { name: "hotel Details", status: false, subheading: 'Add your hotel details.' },
    { name: "location", status: false, subheading: 'Add your hotel location.' },
    { name: "rooms & Amenities", status: false, subheading: 'Add your room details.' },
  ...
  const getHotel = async () => {
    try {
      const { data, statusCode, error, success } = await apiCall(ENDPOINTS.HOTEL_BY_TOKEN, 'GET', {});
  ...
  const getHotelDetails = async (id) => {
    // Fetch hotel details
    const { data, statusCode, error, success } = await apiCall(`${ENDPOINTS.HOTEL_BY_ID}/${id}`, 'GET', {
      // headers: {
    ...
    if (success) {
      ...
      const { hotelName, businessLicense, totalRoom, termsAndConditions, _id } = data.data.hotel;
  ...
  const getHotelLocation = async (id) => {
    const { data, statusCode, error, success } = await apiCall(`${ENDPOINTS.HOTEL_LOCATION}/${id}`, 'GET', {});
    if (success) {
  ...
  const getHotelRoomsAndAmenities = async (id) => {
    const { data: standardRoomData, statusCode: standardRoomStatusCode, error: standardRoomError, success: standardRoomSuccess } = await apiCall(`${ENDPOINTS.HOTEL_ROOM}?hotelId=${id}&roomType=standard`, 'GET', {
      // headers: {
      //   Authorization: `Bearer ${token}`
      // }
    });
    const { data: luxuryRoomData, statusCode: luxuryRoomStatusCode, error: luxuryRoomError, success: luxuryRoomSuccess } = await apiCall(`${ENDPOINTS.HOTEL_ROOM}?hotelId=${id}&roomType=luxury`, 'GET', {
      // headers: {
      //   Authorization: `Bearer ${token}`
      // }
    });
    ...
    if (standardRoomData) {
      const { amenities, hotelId, images: files, numberOfRoom, roomType, roomPrice, _id } = standardRoomData.data;
      ...
      const structuredObject = {
        _id,
        amenities: structuredAmenites, numberOfRooms: numberOfRoom, files, roomPrice
      }
      ...
      if (luxuryRoomData) {
        const { amenities, hotelId, images: files, numberOfRoom, roomType, roomPrice, _id } = luxuryRoomData.data;
        const structuredAmenites = amenities.map((amenity) => ({
          ...amenity,
          checked: amenity.status,
        }))
  ...
  const getAmenitiesList = async () => {
    const { data, statusCode, error, success } = await apiCall(`${ENDPOINTS.GET_AMENITIES}`, 'GET', {});
    if (success) {
  ...
  if (loading) {
    return <div className={styles.loaderParent}><Loader /></div>
  }

  return (
    <div className={styles.formContainer} id={styles.multiStepFormParent} ref={formTopRef}>
      <FormHeader
