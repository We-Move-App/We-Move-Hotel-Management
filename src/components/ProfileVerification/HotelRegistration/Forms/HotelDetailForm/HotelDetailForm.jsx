...
const SUPPORTED_FORMATS = ['image/jpg', 'image/jpeg', 'image/png', 'application/pdf']; // Supported file types
...
const HotelDetailForm = ({ initialValues, onPrev, onNext }) => {
    ...
    const [hotelId, setHotelId] = useState(localStorage.getItem('WEMOVE_HOTELID') || '');
        ...
        onSubmit: async (values, { setErrors, setStatus }) => {
            ...
            try {
                ...
                if (values._id) {
                    ...
                    const { data, statusCode, error, success } = await apiCall(`${ENDPOINTS.HOTEL_BY_ID}/${values._id}`, 'PUT', {
                        body: payloadBody, headers: {
                    ...
                    const { data, statusCode, error, success } = await apiCall(ENDPOINTS.HOTEL_DETAILS, 'POST', {
                        body: payloadBody, headers: {
                            'Content-Type': 'multipart/form-data',
                            Authorization: `Bearer ${JSON.parse(localStorage.getItem('WEMOVE_TOKEN')).accessToken}`
                        }
            ...
            <div className={styles.formFieldsContainer}>
                ...
                <div className={styles.dragDropWrapper}>
                    <div className={styles.dragDropLabelBox}>
                        ...
                        <CustomLabel labelText={"Upload Hotel Photo (min: 3 img)"} required={true} />
                    ...
                    <div className={styles.dragDropBox}>
                        ...
                        {
                            formik.values.files && (<FilesList files={formik.values.files} fileProgress={fileProgress} removeFile={removeFile} minRequiredFiles={3} />)
                        }
                ...
                <CustomCheckBox
                    ...
                    onChange={(e) => formik.setFieldValue("termsAndConditions", e.target.checked)}
                    // onChange={formik.handleChange}
                    required={true}
