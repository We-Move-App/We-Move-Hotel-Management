...
const HotelRoomsForm = ({ initialValues, onPrev, onNext, formTopRef }) => {
  ...
  const [fileProgress, setFileProgress] = useState({ standardRoom: {}, luxuryRoom: {} });
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        if (hotelID) {
          ...
          const { numberOfRooms: standard_numberOfRooms, roomPrice: standard_roomPrice, amenities: standard_amenities, files: standardRoomFiles } = values.standardRoom;
          const { numberOfRooms: luxury_numberOfRooms, roomPrice: luxury_roomPrice, amenities: luxury_amenities, files: luxuryRoomFiles } = values.luxuryRoom;

          const structuredStandardAmenites = standard_amenities.map((amenity) => {
          ...
          standardRoomsForm.append('amenities', JSON.stringify(structuredStandardAmenites));
          ...
          luxuryRoomsForm.append('amenities', JSON.stringify(structuredLuxuryAmenites));
          ...
          if (standardRoomId && luxuryRoomId) {
            ...
            const { data: standardRoomData, statusCode: standardRoomStatusCode, error: standardRoomError, success: standardRoomSuccess } = await apiCall(`${ENDPOINTS.HOTEL_ROOM}?hotelId=${hotelID}&roomType=standard`, 'PUT', {
              body: standardRoomsForm,
            })

            const { data: luxuryRoomData, statusCode: luxuryRoomStatusCode, error: luxuryRoomError, success: luxuryRoomSuccess } = await apiCall(`${ENDPOINTS.HOTEL_ROOM}?hotelId=${hotelID}&roomType=luxury`, 'PUT', {
              body: luxuryRoomsForm,
            })

            if (standardRoomError && luxuryRoomError && (standardRoomError?.message === luxuryRoomError?.message)) {
              setFormError(prev => ({
                ...prev,
            ...
            if (standardRoomData && standardRoomStatusCode === 200 && standardRoomSuccess
              && luxuryRoomData && luxuryRoomStatusCode === 200 && luxuryRoomSuccess) {
            ...
            const { data: standardRoomData, statusCode: standardRoomStatusCode, error: standardRoomError, success: standardRoomSuccess } = await apiCall(`${ENDPOINTS.HOTEL_ROOM}?hotelId=${hotelID}&roomType=standard`, 'POST', {
              body: standardRoomsForm,
            })


            const { data: luxuryRoomData, statusCode: luxuryRoomStatusCode, error: luxuryRoomError, success: luxuryRoomSuccess } = await apiCall(`${ENDPOINTS.HOTEL_ROOM}?hotelId=${hotelID}&roomType=luxury`, 'POST', {
              body: luxuryRoomsForm,
            })
            ...
            if (standardRoomData && standardRoomStatusCode === 201 && standardRoomSuccess
              && luxuryRoomData && luxuryRoomStatusCode === 201 && luxuryRoomSuccess) {
  ...
  const [activeTabBar, setActiveTabBar] = useState(tabBarData[0].status ? 'standardRoom' : 'luxuryRoom');
  ...
  const handleFilesChange = (newFiles) => {
    ...
    formik.setFieldValue(`${activeTabBar}.files`, [...currentFiles, ...newFiles]);
  ...
  const simulateUpload = (file) => {
    ...
    const interval = setInterval(() => {
      ...
      setFileProgress(prev => ({
        ...prev,
        [activeTabBar]: { ...activeTabBar, [file.name]: Math.min(progress, 100) }
  ...
  const removeFile = (index, fileName) => {
    ...
    formik.setFieldValue(`${activeTabBar}.files`, [...files.slice(0, index), ...files.slice(index + 1)]);
    setFileProgress(prev => {
  ...
  const handleRoomTypeChange = () => {
    // window.focus();
    setTabBarData([{ name: 'Standard room', status: false }, { name: 'Luxury room', status: true }])
            ...
            <div className={styles.formFieldsContainer}>
              ...
              <div className={styles.amenitiesContainer}>
                ...
                <div className={styles.amenitiesWrapper}>
                  {formik.values?.standardRoom?.amenities?.map((amenity, index) => (
                    <div className={styles.item} key={index}>
                      <CustomCheckBox
                        ...
                        onChange={(e) => formik.setFieldValue(`standardRoom.amenities[${index}].checked`, e.target.checked)}
              ...
              <div className={styles.dragDropWrapper}>
                ...
                <div className={styles.dragDropLabelBox}>
                  <CustomLabel labelText={'Upload Room Photo (min: 3 img)'} required={true} htmlFor={'dragDropBox'} />
                  {/* <span onClick={handleAddClick}><p>Add more</p></span> */}
                </div>

                <FileUpload onFilesChange={handleFilesChange} accept={'.png,.jpg,.JPEG'} />
                <FilesList files={formik.values.standardRoom.files} fileProgress={fileProgress.standardRoom} removeFile={removeFile} minRequiredFiles={3} />
              ...
              <div className={styles.standardFormBtn}>
                <CustomButton type={'button'} buttonText={'Continue'} buttonSize={'small'} onClick={handleRoomTypeChange} />
              </div>
          ...
          activeTabBar === 'luxuryRoom' && (
            <div className={styles.formFieldsContainer}>
                  ...
                  {formik.values?.luxuryRoom?.amenities?.map((amenity, index) => (
                    <div className={styles.item} key={index}>
                      <CustomCheckBox
                        ...
                        // error={formik?.errors[activeTabBar]?.amenities[index]}
                        // onChange={(e) => formik.setFieldValue(`%${[activeTabBar]}.amenities[${index}].checked`, e.target.checked)}
                        onChange={(e) => formik.setFieldValue(`luxuryRoom.amenities[${index}].checked`, e.target.checked)}
              ...
              <div className={styles.dragDropWrapper}>
                ...
                <FileUpload onFilesChange={handleFilesChange} accept={'.png,.jpg,.JPEG'} />
                <FilesList files={formik.values.luxuryRoom.files} fileProgress={fileProgress.luxuryRoom} removeFile={removeFile} minRequiredFiles={3} />
      ...
      </div>
      {
        formError?.error ? (
          ...
          : (formError?.standardError || formError?.luxuryError) && (
            <div className={styles.formsErrorBox} >
              {
                formError?.standardError && <p className={styles.error}>{`Standard Rooms Form: ${formError?.standardError}`}</p>
              }
              {
                formError?.luxuryError && <p className={styles.error}>{`Luxury Rooms Form: ${formError?.luxuryError}`}</p>
              }
            </div>
          )
      }
