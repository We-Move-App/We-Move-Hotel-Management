...
const Login = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setErrors, setStatus }) => {
            ...
            console.log('Form submitted');
            console.log('Form data:', values);
            const emailOrPhone = values.emailOrPhone.split("").includes("@") ? values.emailOrPhone : "+91" + values.emailOrPhone;
            const payloadBody = {
                emailOrPhone: emailOrPhone,
                password: values.password
            }
            await axios.post(`${BASEURL}${ENDPOINTS.LOGIN}`, payloadBody).then(res => {
                console.log(res)
                ...
                console.log(error)
                error?.response?.data.message && setStatus(error?.response?.data.message || 'not authorized');
                return;
            })
        },
    })
        ...
        <div className={styles.formContainer}>
            ...
            <form onSubmit={formik.handleSubmit} className={styles.form} >
                ...
                <div className={styles.formFieldsContainer}>
                    ...
                    {
                        formik.status && <span className={styles.error} >{formik.status}</span>
                    }
                </div>
                ...
                <div className={styles.formSubmitBtn}>
                    ...
                    <div className={styles.formBottomNoteText}>
                        Don't have an account? <span><Link to="/signup">Sign up</Link></span> here
                    </div>
                </div>
            </form>
        </div>
