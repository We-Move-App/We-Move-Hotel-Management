...
const GooglePlacesAutocomplete = ({ formik }) => {
  ...
  const parseAddressComponents = (result, lat, lng) => {
    ...
    const locality =
      ...
      getComponent(["route"]); // As fallback only

    const pincode = getComponent(["postal_code"]);
    const landmark = getComponent([
      "point_of_interest",
      "premise",
      "establishment",
    ]);
    ...
    formik.setFieldValue("locality", locality); // Will now not duplicate `city`
  ...
  return (
    <div>
      <GoogleMap
        ...
        onClick={onMapClick}
      >
        <Marker position={selected} draggable={true} onDragEnd={onMapClick} />
      </GoogleMap>
      <br />
      <Autocomplete
