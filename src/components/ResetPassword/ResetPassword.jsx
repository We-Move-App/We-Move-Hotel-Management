...
const emailSchema = Yup.object().shape({
    emailOrPhone: Yup.string()
        ...
        .test('emailOrPhone', 'Please enter a valid email or phone number', function (value) {
...
const resetPasswordSchema = Yup.object().shape({
    password: Yup.string()
        ...
        .test('no-leading-space', 'Password cannot start with a space', value => value && value[0] !== ' ')
        .matches(
            /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[!@#$%^&*(),.?":{}|<>]).*$/,
    ...
    confirmPassword: Yup.string()
        ...
        .test('no-leading-space', 'Confirm password cannot start with a space', value => value && value[0] !== ' '),
});
...
const ResetPassword = () => {
    ...
    const handleModalSubmission = async () => {
        try {
            ...
            const { data, error, statusCode, success } = await apiCall(`${ENDPOINTS.VERIFY_OTP_WITHOUT_TOKEN}`, "POST", { body: payload });
    ...
    const sendOtp = async (values) => {
        try {
            ...
            const { data, statusCode, success, error } = await apiCall(`${ENDPOINTS.GET_OTP_RESET_PASSWORD}`, "POST", { body: payload });
            // console.log(data, error);
            setOpen(true);
    ...
    const handleResetPassword = async (values) => {
        ...
        try {
            ...
            const { data, statusCode, success, error } = await apiCall(`${ENDPOINTS.RESET_PASSWORD}`, "PUT", { body: payload });
            console.log(data, error)
        } catch (error) {
            console.log(error);
        }
